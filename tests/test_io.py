...
class TestCreateMLRW(unittest.TestCase):
    ...
    def test_a_write(self):
        # TODO: finish test functions for JSON reading writing.
        dir_name = os.path.abspath(os.path.dirname(__file__))
        libs_path = os.path.join(dir_name, '..', 'libs')
        sys.path.insert(0, libs_path)
        from attributeJSONIO import AttributeJSONWriter

        person = {'label': 'person', 'points': ((65, 45), (420, 45), (420, 512), (65, 512))}
        face = {'label': 'face', 'points': ((245, 250), (350, 250), (350, 365), (245, 365))}

        expected_width = 105    # 350-245 -> attributeJSONIO.py ll 46
        expected_height = 115   # 365-250 -> attributeJSONIO.py ll 49
        expected_x = 297.5      # 245+105/2 -> attributeJSONIO.py ll 53
        expected_y = 307.5      # 250+115/2 > attributeJSONIO.py ll 54

        shapes = [person, face]
        output_file = dir_name + "/tests.json"

        writer = AttributeJSONWriter('tests', 'test.512.512.bmp', (512, 512, 1), shapes, output_file,
        ...
        self.assertEqual('test.512.512.bmp', data_dict['image'], 'imgFilename not correct in .json')
        self.assertEqual(2, len(data_dict['annotations']), 'jsonFilePath contains to less annotations')
